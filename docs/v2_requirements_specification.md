...
### 1.2 背景
**V1の課題:**
- フォームUIが3つの選択肢のみで、RAGの豊富な知識（23ファイル）を活用できていない
- バックエンドAPIが画面数計算のみで、詳細な見積もりができない
- デザインフェーズ、要件定義、スケジュール提案などの総合的なヒアリングが不可能

**V2での解決:**
- チャットUIで柔軟な会話フローを実現
- RAGを直接参照して詳細な見積もりとスケジュールを生成
...
#### 2.1.4 Markdownダウンロード機能
- **要件**: 会話終了後、見積もり・提案書をMarkdown形式でダウンロード
- **詳細**:
  - ダウンロードボタンの表示
  - ファイル名: `見積もり相談_YYYY-MM-DD.md`
  - OpenAIで整形された読みやすいMarkdown
...
#### 2.2.2 RAGベース見積もり生成
- **要件**: バックエンドAPIを使わず、RAGから直接見積もりを生成
- **詳細**:
  - 23個のRAGファイルから関連情報を検索
  - 価格情報（Web/モバイル/デザイン/インフラなど）を参照
  - ユーザーの要件に基づいて詳細な見積もりを計算
...
1. **機能面**: チャットUIで要件定義〜開発の全フェーズをヒアリング可能
2. **品質面**: RAGの23ファイルすべてを活用した詳細な見積もり生成
